...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        product = Product.objects.select_related("category", "created_by").get(
...
class RestockQueueViewSet(viewsets.ModelViewSet):
    ...
    http_method_names = [
        ...
        "delete",
        "post",
    ]  # Only allow GET, DELETE, and POST (for custom actions)
...
# =============================================================================
# DASHBOARD STATS VIEW — MODULE B6
# =============================================================================
...
@extend_schema(
    ...
    description="""
    Single aggregation endpoint for the dashboard KPI cards.

    Returns comprehensive statistics including:
    - Order counts (today, pending, completed today)
    - Revenue for today
    - Low stock product count
    - Product summary with derived status
    ...
    **Field definitions:**
    ...
    - `product_summary`: All products with derived status (low_stock/out_of_stock/ok)
    """,
...
def dashboard_stats(request):
    """
    Dashboard Stats Aggregation Endpoint — Module B6

    GET /api/dashboard/stats/

    Returns KPI statistics for the dashboard including:
    - Orders today count
    ...
    completed_orders_today_count = Order.objects.filter(
        status=Order.STATUS_DELIVERED, created_at__date=today
    ).count()

    # Calculate revenue for today (exclude cancelled orders)
    revenue_today_sum = (
    ...
    # Handle None case if no orders today
    revenue_today = (
        revenue_today_sum if revenue_today_sum is not None else Decimal("0.00")
    )

    # Calculate low stock count (stock_quantity < min_stock_threshold)
    low_stock_count = (
        Product.objects.filter(stock_quantity__lt=F("min_stock_threshold"))
        .exclude(status=Product.STATUS_ARCHIVED)
        .count()
    )
